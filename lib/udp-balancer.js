...
class UdpBalancer {
    constructor(options) {
        ...
        this.family = 'IPv4';
        this.udpType = 'udp4';
        this.consensusNumber = options.consensusNumber;
    ...
    getNextTarget(){
        ...
        this._details.target.currentTarget ++;
        if(this._details.target.currentTarget == this._details.target.address.length) this._details.target.currentTarget = 0;
        return this._details.target.address[this._details.target.currentTarget]
    }
    /**
    ...
    handleProxyMsg(socket, proxy, msg, sender, peer) {
        ...
        if(proxy.replies.hasOwnProperty(peer.address+peer.port)){
            if(proxy.replies[peer.address+peer.port].msg===response.answers[0].address)
                proxy.replies[peer.address+peer.port].replies++
            if(proxy.replies[peer.address+peer.port].replies==proxy.consensusNumber){
            ...
            proxy.replies[peer.address+peer.port]={msg:response.answers[0].address,replies:1}
            if(proxy.consensusNumber===1){
                proxy.send(msg, peer.port, peer.address, function (err, bytes) {
                    if (err)
                        socket.emit('proxyError', err);
                });
    ...
    handleMessage(socket, proxy, msg, sender) { 
        ...
        for(var i=0;i<proxy.consensusNumber;i++){
            const target = proxy.getNextTarget();
            console.log('forwarding to '+ target);
            socket.send(msg, 0, msg.length, proxy.port, target, function (err, bytes) {
                if (err)
                    proxy.emit('proxyError', err);
                if (!socket.t)
